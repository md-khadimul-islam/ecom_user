...
class UserProvider extends ChangeNotifier {
  ...
  getUserSnapshot() {
    DbHelper.getUserSnapshotInfoById(AuthService.currentUser!.uid).listen((
        snapshot) {
  ...
  Future<void> updateUserDisplayName(String value) {
    return DbHelper.updateUserField(
        AuthService.currentUser!.uid, {'displayName': value});
  }

  Future<void> updateUserPhoneNumber(String value) {
    return DbHelper.updateUserField(
        AuthService.currentUser!.uid, {'phoneNumber': value});
  }

  Future<AppUser> getUser() async {
    final docSnapshot = await DbHelper.getUserInfoById(
        AuthService.currentUser!.uid);
    return AppUser.fromJson(docSnapshot.data()!);
  }

  Future<String> uploadImage(String path) async {
    final imageName = 'UserImage_${DateTime.now().millisecondsSinceEpoch}';
